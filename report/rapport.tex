...
\title{Finding Hollywood's most Productive\\Using Map-Reduce (and Streaming?) FIXME}
...
In this section, we attempt to count the most productive actor pairs, ordering them by the number of movies they starred in together. This problem will most likely not be treated, and this section will not be in the final report. 
...
%TODO do something about combine
%motivation MLFA
%description
  %equivalence of problems
  %our algorithm (this is the idea, we made it as a twist on pagh)
  %universal hashing??? move to reference?
%pseudocode MABG
%analysis
  %run-time
    %sketch up pagh analysis
    %mention intersection inside B-loop
