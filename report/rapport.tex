...
\begin{itemize}
  ...
  \item Watch list: $W$: Chosen subset of $\pi_{a}(R_1)$
...
The worst-case running time is the same as \cite{paper:amossen}, but in actual use, we predict that our implementation will be significantly slower. This is due to the wasted work done in the inner loop, whenever $P_i < P_{max}$.

\paragraph{Space Usage}
The algorithm uses $O(|W|k)$ space, since it has to maintain $W$ individual sketches, each of size $O(k)$. It is worth noting, that the space usage is independent of the input size $n$. This is only a factor $|W|$ more than \cite{paper:amossen}, which is to be expected.
  %io
