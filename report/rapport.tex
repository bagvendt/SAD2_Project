...
\title{Finding the most popular actor in the IMDB dataset\\Implementing a Map-Reduce algorithm}
...
In this section, we try to answer the question: "How many unique co-actors has a a given actor starred in at least one movie with", in the following called the \emph{Popular} problem. We will develop and run a Map-Reduce algorithm using the Hadoop framework, both locally and on the Amazon Elastic Cloud service.
\paragraph{}
...
The amount of work done in each mapper/reducer is close to being balanced. Obviously, the round two reducer assigned to the most productive actor in the dataset, will do more work than the reducer assigned to an actor that only star in a single movie, but in terms of the input size, these numbers are roughly equal. This is true because no single actor has played in a significant fraction of movies in the dataset.

% TODO: Should we expand on "tricks", possibly with a ref
If the above was not true, additional techniques, beyond the scope of this project, would be needed to balance the workload.
...
The fact that two algorithms, developed by different people, using different programming languages and different algorithmic paradigms, produce the same output, gives us a very high confidence in both implementations. The odds of introducing the same systematic error into both, are extremely low.
...
Taking one sample from the output we see from Bess Flowers' Wikipedia that she was a Hollywood extra that has appeared in over 700 movies. Maybe it is safe to assume that since she has starred in so many movies she would also be starring with a lot of other actors

We realize that this is not a formal proof that the produces output is correct.

%MABG
\subsection{Benchmark}
We have carried out a small scale benchmark experiment to test the difference in running time between the sequential and the Map-Reduce algorithms, both in a local and in a distributed setup. 

For the local setup we used a 2.5 GHz Intel Core i5 chip (denoted as $Local^*$ in the table). 

In the distributed setup we used the online web service Amazon Elastic Mapreduce that runs a custom version of Hadoop. 
...
\end{table}
We realize that our experiment does not contain enough data to conclude anything, although it illustrates a tendency in the distributed setup that more powerfull instances yields a faster running time and more instances yields a faster running time. 

The experiment also shows that the sequential algorithm in fact yields the fastest running time, we have been able to achieve. 
It is our strong belief that if our algorithms were to be run on a significantly larger dataset we would see that the distributed setups would yield better running times.  


\section{Appendix}
\textbf{Popular.java - Map-Reduce}
\lstinputlisting[language=Java]{../map_reduce/popular/src/Popular.java}
\textbf{Promiscuous.py - Project sql data into text file}
\lstinputlisting[language=Python]{../projections/promiscuous.py}
