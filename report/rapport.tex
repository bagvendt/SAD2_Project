...
\todo{Er subsection nødvendig?}
\subsection{Introduction}

\todo{We should introduce entire report, as the report is no longer seperated into 2 parts}
In this section, we try to answer the question: "How many unique co-actors has a given actor starred alongside", we call this the \emph{Popular} problem. In the following, we will develop and execute a Map-Reduce algorithm using the Hadoop framework, both locally and on the Amazon Elastic Cloud service.
\paragraph{}
...
\vspace{-10pt}
\end{figure}
\todo{Intet nævnt om hvordan data er projiceret. Er billedet nok eller skal det nævnes?}
...
The shuffle step (part of the Hadoop framework)\todo{Specifikt for Hadoop?}, directs each pair emitted by a mapper, to a reducer, such that every pair with a given key $k$, is sent as input to a single reducer $R_k$.\\
...
The algorithm runs in a constant number of rounds (2), independent of the problem size. As mentioned previously, this can be reduced to a single round, if the input has a more suitable structure.\todo{mener jeg ikke vi nævner. Kun at vi har gjort det sværere, men ikke alternativet}
...
\todo{Giver det mening at have med?}In that case the best guarentee we can give for our approximation for each actor, $\tilde{z}_i$, is $\tilde{z}_i \leq z_i |m_i|$, where $m_i$ is that actors total number of movies and $i$ is the actors index. This is the case when an actor has only played with the same group of people.
\\
...
The fact that two algorithms, developed by different people\todo{Måske udelade different people?}, using different programming languages and different algorithmic paradigms, produce the same output, gives us a very high confidence in the correctness of both implementations. The odds of introducing the same systematic error into both, are extremely low.
...
It is our strong belief that if the dataset was larger we would see the distributed setup execute significantly faster than the sequential. \todo{Nævne overhead for distribuerede systemer?}
...
The Popular problem can be expressed as a boolean matrix multiplication. Consider a matrix A of size $|actors| \cdot |movies|$, where the value $1$ at position $(x, y)$ means that actor $x$ starred in movie $y$.\todo{Vil det give en lettere overgang at bruge $m_1$ og $m_2$?} $R = A \cdot A^T$ will then have the dimensions $|actors| \cdot |actors|$, and a $1$ at position $(x, y)$ indicates that actor $x$ and $y$ starred in at least one movie together. Summing the number of non-zero entries in $R_{i,j}$ for a given $i$, gives the number of unique actors that actor $i$ starred with at least once, which is exactly the value we want to compute.\\
Additionally, matrix multiplication can be expressed as the \emph{join-distinct} operation from relational algebra as follows: Consider the boolean matrix multiplication: $R = A \cdot C$ \todo{Hvad er $C$?}. Let the relation $R_1$ be the pairs corresponding to the non-zero values in A, and the $R_2$ be the pairs corresponding to non-zero values in C. Now each unique pair in $R_1 \Join R_2$ will correspond to a non-zero value in R.
...
\begin{itemize}
  ...
  \item $A_i$: $\pi_{a}(\sigma_{b=i}(R_1))$ \todo{skal vi overhovedet have denne når vi har $A_{\pi}$ og $W$}
  \item $A_{\pi}$
  \item $C_i$: $\pi_{c}(\sigma_{b=i}(R_2))$
  \item $W$: Chosen subset of $\pi_{a}(R_1)$ (the watchlist)
  \item $F_s$: Buffer containing between $[0:k]$ observed hash values for a given $s \in B$
  \item $S_s$: Sketch containing the $k$ smallest observed hash values for a given $s \in B$
...
After the check against $p_{max}$, the value of $h$ is checked against $p_s$. If the value of $h$ is lower, we add the pair $(x_s, y_t)$ to the buffer $F_s$. When the current value of $h$ is greater than $p_{max}$ we move on to the next column, starting from the row where we found the minimum value in the previous column. This is more efficient, because it utilizes the fact that the columns are sorted according to $h_2$, limiting the number of iterations needed. When a buffer is filled, it is merged into the corresponding sketch, using the \texttt{COMBINE} procedure outlined in \cite{paper:amossen}\todo{Tror det er bedre selv at forklare hvad COMBINE gør i stedet for at henvise}. Once all $i \in B$ have been processed, we approximate \~{z}$_s$ by utilizing the result from Bar-Yossef et al. giving us: \~{z}$_s$ = $k/p_s$
...
\todo{lille $p_s$ istedet for stort $P_s$?}
\begin{algorithm}[H]
\DontPrintSemicolon
  \Begin{
    $p_{max}$ $\leftarrow$ $p$ \;
...
Following \cite{paper:amossen}, we split the analysis of the running time into two.\todo{Skal vi skrive de følger samme fremgangsmåde?}
...
Updating $P_{max}$ on line 20 can be done in $log(|W|)$ time, using a Heap\todo{Nogle negative sider, siden det ikke bliver brugt i combine?}.
...
If we wish to increase the confidence in the approximation, the error-probability $(2/3)$ can be made arbitrarily small, by rerunning the algorith a number of times, and choosing the median.\todo{Ville gerne have formlen med, men kan ikke finde den}
...
We have run the experiment twice, 100 for $k=256$ and 100 for $k=1024$, to see the effect of choice of $k$ on the error ratio. \todo{Error eller Variance?}
